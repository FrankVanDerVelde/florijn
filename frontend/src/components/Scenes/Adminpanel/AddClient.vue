    ...
    <div class="profile-container flex self-center w-4/5">
      <div class="grid grid-cols-3 gap-4">
        <form>
          ...
          <input class="rounded border-[1px] border-neutral-200" :value="name" type="text" placeholder="naam bedrijf" id="bedrijfsnaam" name="name"><br><br><br>

          <label class="font-medium">Gegevens</label><br>
          <input class="rounded border-[1px] border-neutral-200" type="text" :value="email" placeholder="e-mail" name="email"><br>
          <input class="rounded border-[1px] border-neutral-200" type="password" :value="password" placeholder="wachtwoord" name="password"><br><br><br>

          <button id="confirm" :title="addClient" class="bg-primary-500 font-bold border-[1px] w-[121px] h-[38px] rounded-md text-neutral-0">Bevestigen</button>
      ...
      <div>
        ...
        <input type="file" :value="profilePictureURL" class="pl-4" accept="image/png, image/gif, image/afiv, image/svg, image/webp, image/jpeg" name="profilePictureURL">
      </div>
      </div>
      <div class="grow">
...
export default {
  ...
  methods: {
    ...
    async addClient(){
      ...
      if (this.addClient) {
        await this.fetchService.fetchJsonPost(`/users/save/client`, input)
      }
    }
  }
}
