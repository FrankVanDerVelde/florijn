<template>
    <Listbox as="div" @update:modelValue="$emit('update:modelValue', $event)" :value="modelValue">
        ...
        <span class="flex items-center">
          <Asset v-if="modelValue != null"
                 :src="modelValue.avatarUrl ?? 'https://icons.veryicon.com/png/o/miscellaneous/bottom-navigation-bar/home-page-not-selected.png'"
                 class="h-6 w-6 flex-shrink-0 rounded-full"/>
          <span class="ml-3 block truncate" :class="{'opacity-50 font-medium': modelValue.name == null}">{{ modelValue.name ?? "Selecteer een klant" }}</span>
        </span>
