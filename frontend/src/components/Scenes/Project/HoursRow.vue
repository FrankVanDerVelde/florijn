<template>
    ...
    <tr @click="showPopup = true">
        <td>
            <small-table-card v-if="showProject" :title="registry.project.title" :subtitle="registry.participant.role" :src="registry.project.logoSrc"
                              :round-image="false"/>
...
export default {
    ...
    methods: {
        async updateRegistryStatus(accepted) {
            const response = accepted ? await this.hourRegistrationRepository.acceptHourRegistration(this.registry.id)
                : await this.hourRegistrationRepository.rejectHourRegistration(this.registry.id);
